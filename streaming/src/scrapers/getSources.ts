    ...
    wbg: {
      ...
      __wbg_eval_c824e170787ad184: function () {
        return applyToWindow(function (index: number, offset: number) {
          ...
          let value;
          if (fake_str === 'fake_window.navigator.webdriver') {
            value = fake_window.navigator.webdriver;
          } else if (fake_str === 'fake_window.navigator.userAgent') {
            value = fake_window.navigator.userAgent;
          } else if (fake_str === 'fake_window.ReactNativeWebView') {
            value = fake_window.ReactNativeWebView;
          } else if (fake_str.startsWith('fake_window.pid = ')) {
            const pid = fake_str
              .split('fake_window.pid = ')[1]
              .replace(/['"]+/g, '');
            value = fake_window.pid = pid;
          } else if (
            fake_str.startsWith("fake_window.localStorage.setItem('kversion', ")
          ) {
            const kversion = fake_str
              .split("fake_window.localStorage.setItem('kversion', ")[1]
              .replace(/['")]+/g, '');
            value = fake_window.localStorage.setItem('kversion', kversion);
          } else if (
            fake_str.startsWith("fake_window.localStorage.setItem('kid', ")
          ) {
            const kid = fake_str
              .split("fake_window.localStorage.setItem('kid', ")[1]
              .replace(/['")]+/g, '');
            value = fake_window.localStorage.setItem('kid', kid);
          } else if (
            fake_str.startsWith("fake_window.localStorage.setItem('ktime', ")
          ) {
            const ktime = fake_str
              .split("fake_window.localStorage.setItem('ktime', ")[1]
              .replace(/['")]+/g, '');
            value = fake_window.localStorage.setItem('ktime', ktime);
          }
...
async function loadWasm(url: any) {
  const mod = initWasm();
  const response = Bun.file('./megacloud.wasm');
